...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : lun. 15 avr. 2024 à 11:48
...
('CMDR1', '2024-04-03', '10002');

--
-- Déclencheurs `abonnement`
--
DROP TRIGGER IF EXISTS `orderAboIdCondition`;
DELIMITER $$
CREATE TRIGGER `orderAboIdCondition` BEFORE INSERT ON `abonnement` FOR EACH ROW BEGIN
...
DELIMITER $$
CREATE TRIGGER `deleteOrder` BEFORE DELETE ON `commande` FOR EACH ROW BEGIN
...
INSERT INTO `commandedocument` (`id`, `nbExemplaire`, `idLivreDvd`, `statut`) VALUES
('CMDD1', 45, '20003', 'Relancée'),
('CMDL1', 5, '00017', 'En cours');

--
-- Déclencheurs `commandedocument`
--
DROP TRIGGER IF EXISTS `orderDocIdCondition`;
DELIMITER $$
CREATE TRIGGER `orderDocIdCondition` BEFORE INSERT ON `commandedocument` FOR EACH ROW BEGIN
    DECLARE count_records INT;
    SELECT COUNT(*) INTO count_records 
...
CREATE TABLE IF NOT EXISTS `service` (
  `nomService` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL,
...
INSERT INTO `suivi` (`statut`) VALUES
('En cours'),
('Livrée'),
('Réglée'),
('Relancée');
...
CREATE TABLE IF NOT EXISTS `utilisateur` (
  ...
  `nomService` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
  PRIMARY KEY (`id`),
...
INSERT INTO `utilisateur` (`id`, `identifiant`, `password`, `nomService`) VALUES
...
ALTER TABLE `commandedocument`
  ...
  ADD CONSTRAINT `commandedocument_ibfk_3` FOREIGN KEY (`statut`) REFERENCES `suivi` (`statut`) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
ALTER TABLE `utilisateur`
  ADD CONSTRAINT `utilisateur_ibfk_1` FOREIGN KEY (`nomService`) REFERENCES `service` (`nomService`);
COMMIT;
